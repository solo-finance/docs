...
- **Example rows.** Rows whose identifier column starts with **`Ex.`** are
  ...
  category: `File #` for data workbooks, `Subnetwork #` for subnetworks, and the
...
- **Blank rows.** Rows missing their identifying values entirely — a data row
  with no file number and no subnetwork name, a subnetwork row with no
  subnetwork name, a policy row with no policy name — are treated as empty padding and
  skipped.
