...
<Steps>
  <Step title="A WorkOS API key for your organization">
    ...
    <Warning>
      Treat the API key like any other credential. Store it in a secrets manager and never commit it to source control.
    </Warning>
  </Step>
  <Step title="The email you use to sign in to SOLO">
    Your email is the SFTP username. The local part can be anything your WorkOS org allows; the domain identifies the organization the upload is attributed to.
  </Step>
...
Each environment writes to its own storage bucket and database; there is no crossover.
...
On a successful connection you'll land at the root of your organization's home directory. List it to see the five category folders waiting for uploads:

```bash
sftp> ls
kyb_cert_policy   kyb_furnish_data   kyc_cert_policy   kyc_furnish_data   programs
```

<Tip>
  If the server rejects you immediately, the most common causes are (1) the API key has been revoked, (2) the email isn't associated with an organization in SOLO yet, or (3) the plus-tag is misspelled (only `+sandbox`, `+prod`, or no tag are accepted). Re-check the username and key, then reach out to your account manager if it persists.
</Tip>

## Step 3 — Download a template

Every upload is an Excel workbook (`.xlsx`) that lands in one of five category directories. Workbook templates are available on the [Upload Categories](/sftp/schemas) page — they include the required header row, an example row, and the right column names already in place.
...
A few things to know about the workbook format ([full reference](/sftp/csv-format)):
...
Ingestion is asynchronous — the file is picked up within a few seconds of landing, and a small workbook typically completes processing in well under a minute.

The cleanest way to check is the [SOLO dashboard](https://app.solo.one): the **Data → Uploads** page lists every workbook your organization has sent, along with row-level success and error counts. From there you can also drill into the resulting records (policies, programs, furnish events) to confirm they showed up where you expected.

<Note>
  Re-uploading the same workbook is safe. The ingester deduplicates based on file content, so retrying after a transient failure won't double-create records.
</Note>

## Common first-upload issues

<AccordionGroup>
  <Accordion title="Authentication failed / Permission denied">
    Confirm the username is your full email address (with `+sandbox` for sandbox), and the password is the literal WorkOS API key string — no `Bearer` prefix, no quotes. If you rotated the key recently, the old value is rejected immediately.
  </Accordion>
  ...
  <Accordion title='Row rejected with "value is None" on a formula cell'>
    Workbooks are read with cached values only. If a workbook was generated programmatically and never opened in Excel, formula cells may have no cached result. Open and save the file in Excel (or set cached values when generating it) before uploading.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="SFTP Overview" icon="server" href="/sftp/overview">
    The full mental model — categories, environments, how routing works.
  </Card>
  <Card title="Upload Categories" icon="list" href="/sftp/schemas">
    Required and optional columns for every category, plus downloadable templates.
  </Card>
  <Card title="Workbook Format" icon="file-excel" href="/sftp/csv-format">
