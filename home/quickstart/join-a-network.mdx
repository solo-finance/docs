...
- **Programs** — partitions inside a network that separate furnishers; you name
  the program when furnishing, and it participates in
  [furnishing policy](/api-overview/furnishing/overview) resolution.
- **Subnetworks** — narrower trust boundaries nested under a parent network.

Both are explained in [Networks](/concepts/governance/networks), and the rules
