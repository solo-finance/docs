...
A [product](/concepts/querying/products) is the unit of data you furnish or
...
[coverage](/concepts/querying/coverage-check) for a product before you commit
to a billable query.
...
3. **Furnish.** Contribute data through a product's `furnish` endpoint — or in
   bulk via [file upload](/concepts/furnishing/file-upload) or
   [SFTP](/concepts/sftp/overview). The network's furnishing policy validates
   what you send.
...
[Furnishing](/concepts/furnishing/overview) into
[file upload](/concepts/furnishing/file-upload) and the
[SFTP guide](/concepts/sftp/getting-started) for bulk ingestion; if it
primarily consumes data, continue from
[Querying](/concepts/querying/overview) into the individual product pages —
