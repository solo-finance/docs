...
<CardGroup cols={2}>
  ...
  <Card title="Products" icon="box">
    Query and furnish [products](/concepts/querying/products) — KYC/KYB certificates and
    screening lists.
  </Card>
  ...
  <Card title="Bulk ingestion" icon="file-import">
    Upload files for ingestion. See also the [SFTP guide](/concepts/sftp/overview).
  </Card>
</CardGroup>
...
- **Querying** a product requires a `consent_id` and one or more `network_policy`
  pairs. See [Querying](/concepts/querying/overview).
- **Furnishing** a product requires a `network_id`, `program_name`, and
  `application_date`. See [Furnishing](/concepts/furnishing/overview).
