...
New to the platform? Read [High-level concepts](/concepts/overview) first — the
...
<CardGroup cols={2}>
  <Card title="Consent" icon="file-signature">
    Create and read [consent](/concepts/consent) records for consumers and
    businesses. Required before querying.
  </Card>
  <Card title="Entities" icon="users">
    Search consumers and businesses within a [network](/concepts/networks).
  </Card>
  <Card title="Products" icon="box">
    Query and furnish [products](/concepts/products) — KYC/KYB certificates and
    screening lists.
  </Card>
  <Card title="Policies" icon="sliders">
    Create and configure querying [policies](/concepts/policies), and list
    furnishing policies.
  </Card>
  ...
  <Card title="Bulk ingestion" icon="file-import">
    Upload files for ingestion. See also the [SFTP guide](/sftp/overview).
  </Card>
</CardGroup>
...
- **Products** expose `query` (read) and `furnish` (write) operations under
  `/products/{product}/…`. Some also support `furnish_via_upload`.
- **Querying** a product requires a `consent_id` and one or more `network_policy`
  pairs. See [Querying](/concepts/querying).
- **Furnishing** a product requires a `network_id`, `program_name`, and
  `application_date`. See [Furnishing](/concepts/furnishing).
- Errors follow a consistent JSON shape — see [Errors](/errors).
