...
[REST channels](/concepts/furnishing/overview). SFTP is a transport, not a
...
See [Upload Categories](/concepts/sftp/schemas) for the expected columns in
each category.
...
3. **Parse.** The workbook is read: first sheet only, row 1 treated as a
   ...
   ([format details](/concepts/sftp/csv-format)).
...
See [Workbook Format](/concepts/sftp/csv-format) for the full rules, including
...
- The five directories map one-to-one onto the `slug` values accepted by the
  [bulk file upload endpoint](/concepts/furnishing/file-upload)
...
[getting-started guide](/concepts/sftp/getting-started) walks through a first
upload end to end.
...
<CardGroup cols={2}>
  <Card title="Getting Started" icon="rocket" href="/concepts/sftp/getting-started">
  ...
  <Card title="Workbook Format" icon="file-excel" href="/concepts/sftp/csv-format">
    Header normalization, data types, and formatting rules.
  </Card>
  <Card title="Furnishing Overview" icon="arrow-up-from-bracket" href="/concepts/furnishing/overview">
