...
Every upload is an Excel workbook (`.xlsx`) that lands in one of five category directories. Workbook templates are available on the [Upload Categories](/concepts/sftp/schemas) page — they include the required header row, an example row, and the right column names already in place.
...
A few things to know about the workbook format ([full reference](/concepts/sftp/csv-format)):
...
<CardGroup cols={2}>
  <Card title="SFTP Overview" icon="server" href="/concepts/sftp/overview">
