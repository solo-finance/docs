...
[category](/concepts/sftp/schemas). This page is the authoritative reference
...
- **Missing required headers reject the whole workbook.** The error lists
  ...
  re-upload. See [Upload Categories](/concepts/sftp/schemas) for which headers
...
<Tip>
  ...
  required/optional markers in the [category reference](/concepts/sftp/schemas).
</Tip>
...
| Misspelled header (`plicy_name`) | Treated as unknown column; workbook rejected for the missing required header | Copy headers from the [templates](/concepts/sftp/schemas) |
...
<AccordionGroup>
  ...
  <Accordion title="How do I check what failed?">
    ...
    [upsert on natural keys](/concepts/sftp/overview#what-happens-on-upload),
    so corrected rows update rather than duplicate.
  </Accordion>
</AccordionGroup>

For the exact columns each category expects, continue to
[Upload Categories](/concepts/sftp/schemas).
