...
[verification](/concepts/furnishing/overview) event, its supporting evidence,
...
[product query](/concepts/querying/products) assembles trust assets into a
certificate.
...
| **Consolidation** | How is it delivered for reuse? | A [certificate](/concepts/querying/products) (`certificate_id`, with its as-of date) |

A data aggregator returns the first row of that table. SOLO returns all of it.

## A trust asset, concretely

Here is one populated sub-product from a [KYB certificate](/concepts/querying/kyb-certificate)
...
1. **Create.** An institution [furnishes](/concepts/furnishing/overview) the
...
3. **Reuse.** A [product query](/concepts/querying/products) consolidates the
   eligible trust assets into a [certificate](/concepts/querying/kyc-certificate).
...
| **Trust products** | The outcomes a query delivers | [Products](/concepts/querying/products), [KYC](/concepts/querying/kyc-certificate), [KYB](/concepts/querying/kyb-certificate) |
