...
flowchart LR
    subgraph audit [Audit namespace]
        QE[QueryEvent]
        FE[FurnishEvent]
        QER[QueryEventEntitlement]
        FER[FurnishEventEntitlement]
    end
...
| `queryEventEntitlements` | Junction rows linking the event to profiles and every object touched |
...
| `furnishEventEntitlements` | Junction rows linking the run to profiles and furnished objects |
...
`QueryEventEntitlement` and `FurnishEventEntitlement` are junction tables that answer: *"this audit event touched this profile / this object."*
...
2. **Reverse lookups** through `QueryEventEntitlement` and `FurnishEventEntitlement`, filtered to audit events your entity can see
